...
# 创建模型实例
model = GATGCN(gcn_node_dim, gat_node_dim, edge_in_channels, hidden_channels, gcn_num_layers, gat_num_layers, dropout, device).to(device)
...
# 定义损失函数和优化器
initial_lr = 0.000015  # 学习率 0.00005******************************************************************************************
epochs = 60  # 训练轮数
...
for epoch in range(1, epochs + 1):
    ...
    if test_accuracy < best_loss:
        ...
        torch.save(model.state_dict(), "/new/songguanjie/GeoPoc/check_point/demo6_GCNTrans/best_model1.pt")
    print(
...
model.load_state_dict(torch.load( "/new/songguanjie/GeoPoc/check_point/demo6_GCNTrans/best_model1.pt"))
model.eval()
...
df.to_csv( "/new/songguanjie/GeoPoc/check_point/demo6_GCNTrans/Roc1.csv", index=False)
