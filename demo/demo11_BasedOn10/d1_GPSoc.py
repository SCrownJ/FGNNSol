...
class Graph_encoder(nn.Module):
    def __init__(self, node_in_dim, edge_in_dim, hidden_dim,
        ...
        self.layers = nn.ModuleList(
            GNNLayer(num_hidden=hidden_dim, dropout=drop_rate, num_heads= 2) # ************************************************************************************************
    ...
    def forward(self, h_V, edge_index, h_E, seq, batch_id):
        ...
        h_V = F.leaky_relu(h_V)
        h_V = F.leaky_relu(self.W_v(h_V))

        h_E = self.edge_embedding(h_E)
        h_E = F.leaky_relu(h_E)
        h_E = F.leaky_relu(self.W_e(h_E))
...
class GPSoc(nn.Module):  # Geometry-aware Protein Sequence optimal condition predictor
    def __init__(self, node_input_dim=2753, edge_input_dim=450, hidden_dim=256, num_layers=2, dropout=0.2, device='cpu',
                 task='temp'):
        ...
        self.attention = Attention(hidden_dim, dense_dim=16, n_heads=2) # ***************************************************************************************

        # self.task = task
        # self.output_dim = 1

        # self.add_module("FC_1", nn.Linear(hidden_dim, hidden_dim // 2, bias=True))
        ...
        # self.add_module("dropout2", nn.Dropout(dropout))  # 第二个 Dropout 层
        # self.add_module("FC_2", nn.Linear(hidden_dim // 4, self.output_dim, bias=True))
    ...
    def forward(self, h_V, h_E, edge_index, batch_id):
        ...
        # h_V = self._modules["dropout1"](h_V)  # 应用第一个 Dropout
        # h_V = F.leaky_relu(self._modules["FC_a"](h_V))
        # h_V = self._modules["dropout2"](h_V)  # 应用第二个 Dropout
